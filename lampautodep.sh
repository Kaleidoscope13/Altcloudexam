...
sudo mysql -uroot -e "CREATE DATABASE Drum;"
sudo mysql -uroot -e "CREATE USER 'uncle'@'localhost' IDENTIFIED BY 'sam';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Drum.* TO 'uncle'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Drum/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=uncle/' /var/www/laravel/.env
sudo sed -i '27 s/=/=sam/' /var/www/laravel/.env
